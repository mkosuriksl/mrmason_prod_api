...
import com.application.mrmason.service.impl.MaterialSupplierQuotationLoginService;
import com.application.mrmason.service.impl.MaterialSupplierQuotationUserService;
...
@Slf4j
@RestController
public class MaterialSupplierQuotationLoginController {

	@Autowired
	MaterialSupplierQuotationLoginService loginService;
	...
	@Autowired
	MaterialSupplierQuotatuionLoginDAO servicePersonDao;

	ResponseMessageDto response = new ResponseMessageDto();

	@PostMapping("/ms-send-email-otp")
	public ResponseEntity<ResponseMessageDto> sendEmail(@RequestBody OtpSendRequest login) {
		...
		MaterialSupplierQuotationLogin user = servicePersonDao.findByEmailAndRegSource(email, login.getRegSource())
	...
	public ResponseEntity<ResponseMessageDto> verifyUserEmail(@RequestBody OtpVerificationRequest login) {
		try {
			...
			if (optVerify.getEOtp() != null && !optVerify.getEOtp().equals(login.getOtp())) {
				...
				MaterialSupplierQuotationUser user = userDAO.findByEmailAndRegSource(login.getContactDetail(), login.getRegSource()).orElseThrow(
						() -> new ResourceNotFoundException("User Not Found By : " + login.getContactDetail()));
	...
	@PostMapping("/ms-send-mobile-otp")
	public ResponseEntity<ResponseMessageDto> sendSms(@RequestBody OtpSendRequest login) {
		String mobile = login.getContactDetail();
		MaterialSupplierQuotationLogin user = servicePersonDao.findByMobileAndRegSource(mobile, login.getRegSource())
				.orElseThrow(() -> new ResourceNotFoundException("User Not Found By : " + login.getContactDetail()));
	...
	@PostMapping("/ms-verify-mobile-otp")
	public ResponseEntity<ResponseMessageDto> verifyMobile(@RequestBody OtpVerificationRequest login) {
		try {
			...
			if (optVerify.getMOtp() != null && !optVerify.getMOtp().equals(login.getOtp())) {
				...
				MaterialSupplierQuotationUser userStatus = userDAO.findByMobileAndRegSource(login.getContactDetail(), login.getRegSource()).orElseThrow(
						() -> new ResourceNotFoundException("User Not Found By : " + login.getContactDetail()));
