...
public interface ServicePersonLoginDAO extends JpaRepository<ServicePersonLogin, Long> {
	...
	@Query("SELECT s FROM ServicePersonLogin s WHERE s.email = :email AND s.regSource = :regSource ORDER BY s.otpSeqNo DESC")
	List<ServicePersonLogin> findByUserEmailAndRegSource(@Param("email") String email, @Param("regSource") RegSource regSource);

	@Query("SELECT s FROM ServicePersonLogin s WHERE s.mobile = :mobile AND s.regSource = :regSource ORDER BY s.otpSeqNo DESC")
