...
import com.application.mrmason.repository.SPCarpentaryConstructionTasksManagementRepository;
...
@Service
public class SPCarpentaryConstructionTasksManagementServiceImpl implements SPCarpentaryConstructionTasksManagementService {

	@Autowired
	private SPCarpentaryConstructionTasksManagementRepository repository;
	...
	@Override
	public List<SPCarpentaryConstructionTasksManagement> createAdmin(RegSource regSource, SPCarpentaryConstructionTaskRequestDto requestDTO)throws AccessDeniedException {
	    ...
	    List<SPCarpentaryConstructionTasksManagement> resultList = new ArrayList<>();

	    for (SPCarpentaryConstructionTasksManagement task : requestDTO.getTasks()) {
	...
	@Override
	public List<SPCarpentaryConstructionTasksManagement> updateAdmin(RegSource regSource, List<SPCarpentaryConstructionTasksManagement> taskList) throws AccessDeniedException {
	    UserInfo userInfo = getLoggedInSPInfo(regSource);
	    if (!UserType.Developer.name().equals(userInfo.role)) {
	        throw new AccessDeniedException("Only Developer users can access this API.");
	    }
	    List<SPCarpentaryConstructionTasksManagement> updatedTasks = new ArrayList<>();
	    for (SPCarpentaryConstructionTasksManagement task : taskList) {
	        ...
	        SPCarpentaryConstructionTasksManagement existingTask = repository.findById(taskIdKey)
	...
	public List<TaskResponseDto> getTaskDetails(String serviceCategory, String taskId, String taskName,RegSource regSource)throws AccessDeniedException {
	    ...
	    Map<String, List<SPCarpentaryConstructionTasksManagement>> grouped = records.stream()
	    ...
	    for (Map.Entry<String, List<SPCarpentaryConstructionTasksManagement>> entry : grouped.entrySet()) {
	        List<SPCarpentaryConstructionTasksManagement> groupRecords = entry.getValue();

	        // Get first record as a template for serviceCategory, taskId, taskName
	        SPCarpentaryConstructionTasksManagement first = groupRecords.get(0);
